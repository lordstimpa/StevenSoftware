              ...
              <MenuItems class="absolute right-0 mt-2 w-44 bg-white border border-slate-200 rounded-md shadow-md p-1 z-20 focus:outline-none">
                <MenuItem v-slot="{ active }">
                  <button @click="goToEdit"
                          class="w-full text-left px-3 py-2 rounded-md text-slate-800 text-sm"
                ...
                <MenuItem v-slot="{ active }">
                  <button @click="showModal = true"
                          class="w-full text-left px-3 py-2 rounded-md text-red-600 text-sm"
...
<script setup>
  ...
  function goToEdit() {
    router.push(`/createblog?mode=edit&id=${blogPostId}`);
  }
